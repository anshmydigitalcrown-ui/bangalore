    ...
    <div className="min-h-screen bg-gradient-to-br from-black via-gray-900 to-black">
        ...
        {mobileMenuOpen && (
          <div className="md:hidden absolute top-full left-0 right-0 bg-black border-b-2 border-deep-red shadow-lg">
            <div className="px-4 py-4 space-y-4">
              <Link href="/" onClick={() => setMobileMenuOpen(false)} className="block text-white hover:text-deep-red transition-colors py-2">Home</Link>
              ...
              <Link href="/gallery" onClick={() => setMobileMenuOpen(false)} className="block text-deep-red font-semibold py-2">Gallery</Link>
              <Link href="/#contact" onClick={() => setMobileMenuOpen(false)} className="block text-white hover:text-deep-red transition-colors py-2">Contact</Link>
              <Link href="/#contact" className="w-full bg-deep-red hover:bg-dark-red text-white px-6 py-3 rounded-lg transition-colors shadow-lg flex items-center justify-center gap-2 mt-4">
                <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z"/>
                </svg>
                Call Now
      ...
      <section className="relative py-12 sm:py-16 lg:py-20 px-4 sm:px-6">
        <div className="max-w-7xl mx-auto text-center">
          ...
          <p className="text-base sm:text-lg lg:text-xl text-gray-300 max-w-3xl mx-auto mb-8 sm:mb-12 leading-relaxed px-4">
          ...
          {/* Category Filter */}
          <div className="flex flex-wrap justify-center gap-2 sm:gap-3 lg:gap-4 mb-12 sm:mb-16 px-4">
            {categories.map((category) => (
              <button
                key={category}
                onClick={() => setSelectedCategory(category)}
                className={`px-3 sm:px-4 lg:px-6 py-2 sm:py-3 rounded-full border-2 transition-all duration-300 hover:scale-105 text-xs sm:text-sm lg:text-base ${
        ...
        {/* Decorative elements */}
        <div className="absolute top-10 sm:top-20 left-4 sm:left-10 w-16 sm:w-24 lg:w-32 h-16 sm:h-24 lg:h-32 bg-gradient-to-r from-deep-red/20 to-transparent rounded-full blur-xl"></div>
        <div className="absolute bottom-10 sm:bottom-20 right-4 sm:right-10 w-12 sm:w-16 lg:w-24 h-12 sm:h-16 lg:h-24 bg-gradient-to-l from-dark-red/20 to-transparent rounded-full blur-xl"></div>
      </section>
        ...
        <div className="max-w-7xl mx-auto">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-6">
            {filteredImages.map((image) => (
              <div
                key={image.id}
                className="group bg-gradient-to-br from-black via-gray-900 to-deep-red/10 rounded-xl sm:rounded-2xl overflow-hidden border border-deep-red/30 hover:border-deep-red transition-all duration-300 hover:scale-105 shadow-xl hover:shadow-2xl hover:shadow-deep-red/20 cursor-pointer"
              ...
              >
                <div className="relative aspect-[3/4] overflow-hidden">
                  ...
                  <div className="absolute bottom-2 sm:bottom-4 left-2 sm:left-4 right-2 sm:right-4 transform translate-y-4 group-hover:translate-y-0 transition-transform duration-300 opacity-0 group-hover:opacity-100">
                    <h3 className="text-white font-bold text-sm sm:text-base lg:text-lg mb-1">{image.title}</h3>
                    <p className="text-gray-300 text-xs sm:text-sm">{image.category}</p>
                  </div>
                </div>
              </div>
            ))}
